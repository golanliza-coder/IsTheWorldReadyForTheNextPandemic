...
* **`WHO-COVID-19-global-daily-data1.csv` & `epidemiology_small_file.csv`**: Time-series records of daily new and cumulative COVID-19 cases, deaths, and testing outputs.
* **`vaccinations_small_file.csv`**: Longitudinal data tracking vaccine rollout doses, individuals vaccinated, and specific vaccine manufacturers (Pfizer, Moderna, Janssen, etc.).
...
3. **Feature Engineering**: Creating cumulative tracking metrics, calculating lagging indicators for pandemic peaks, and computing per capita metrics to standardize features across countries.
4. **Predictive Modeling**: 
   * *Task 1 (Regression)*: Predicting Country Life Expectancy based on health infrastructure and economic metrics.
   * *Task 2 (Classification/Clustering)*: Segmenting countries into pandemic resilience tiers based on GHS indicators and empirical COVID-19 statistics.
